package in.focalworks.ui.components;
...
public class AppNavigation extends PolymerTemplate<TemplateModel> implements AfterNavigationObserver {
	...
	public void init(final List<PageInfo> pages, final String defaultHref, final String logoutHref) {
		this.logoutHref = logoutHref;
		this.defaultHref = defaultHref;

		for (final PageInfo page : pages) {
			final Tab tab = new Tab(new IronIcon("vaadin", page.getIcon()), new Span(page.getTitle()));
	...
	public void afterNavigation(final AfterNavigationEvent event) {
		final String href = event.getLocation().getFirstSegment().isEmpty() ? defaultHref
