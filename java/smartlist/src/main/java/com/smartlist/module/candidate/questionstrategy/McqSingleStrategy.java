...
public class McqSingleStrategy extends BaseQuestionStrategy{

    public McqSingleStrategy(final SubmitAnswerReqDTO submitAnswerReqDto, final Question question,
    ...
    @Override
    public int score(){
        int answerId = this.submitAnswerReqDto.getAnswerIds().get(0);

        final AtomicInteger score = new AtomicInteger();
        this.question.getAnswerOptions().forEach(answerOption -> {
            if (answerOption.isCorrect() && answerOption.getId() == answerId) {
                score.set(this.question.getScore());
            }
        });
        return score.get();
    }
