...
export class AssessmentController {
  ...
  async getById(@Param('id') id: string): Promise<AssessmentRespDto> {
  ...
  async getByToken(@Param('token') token: string): Promise<AssessmentRespDto> {
  ...
  async duplicate(@Param('id') id: string): Promise<string> {
  ...
  @HttpCode(200)
  @Get(AssessmentController.rootRoute)
  async search(
  ...
  async getAssessmentStatusCount(
    ...
    return await this.assessmentService.getAssessmentStatusCountForOrganization(
      organizationId,
    );
  }
